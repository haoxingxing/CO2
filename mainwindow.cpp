...
void MainWindow::readMsg()
{
	...
	case '0':
		ui->msg_2->append(
			"[" + p2p_msg->getSocket()->peerAddress().toString() + "]: " + QByteArray::fromBase64(data.mid(1)));
		break;
	case '1':
		{
...
void MainWindow::send_voice()
{
	...
	if (ui->isaecon->isChecked())
	{
		...
		p2p->getSocket()->write(QByteArray(reinterpret_cast<const char*>(m_AECBufferOut),
		                                   m_AECBufferOut_size * static_cast<int>(sizeof(spx_int16_t))));
	}
	else
	{
...
void MainWindow::switchAudioSample(int target)
{
	...
	ui->label_2->setText(
		QString::number(format.sampleRate() / 1000.0) + "kHz " + QString(
			ui->isaecon->isChecked() ? "AEC ON" : "AEC OFF"));
	log("Init Qt Audio");
